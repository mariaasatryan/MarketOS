  ...
  async getOrders(dateRange: DateRange): Promise<Order[]> {
    try {
      ...
      return postings.flatMap((posting: any) =>
        (posting.products || []).map((product: any) => ({
          id: `${posting.posting_number}_${product.sku}`,
          marketplace: 'ozon' as const,
          sku: product.sku,
          productName: product.name,
          quantity: product.quantity,
          price: product.price || 0,
          status: this.mapOrderStatus(posting.status),
          date: new Date(posting.created_at || posting.in_process_at),
